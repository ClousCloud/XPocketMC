...
abstract class BaseNbtWorldData implements WorldData{
	...
	/**
	 * Hack to fix worlds broken previously by older versions of xpocketmc-MP which incorrectly saved classpaths of
	...
	protected static function hackyFixForGeneratorClasspathInLevelDat(string $className) : ?string{
		...
		switch($className){
			...
			case 'xpocketmc\level\generator\normal\Normal':
				return "normal";
			/** @noinspection ClassConstantCanBeUsedInspection */
			case 'xpocketmc\level\generator\Flat':
				return "flat";
		}
