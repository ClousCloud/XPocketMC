...
use xpocketmc\block\tile\TileFactory;
...
use xpocketmc\entity\EntityFactory;
use xpocketmc\entity\Location;
use xpocketmc\entity\object\ExperienceOrb;
use xpocketmc\entity\object\ItemEntity;
use xpocketmc\event\block\BlockBreakEvent;
use xpocketmc\event\block\BlockPlaceEvent;
use xpocketmc\event\block\BlockUpdateEvent;
use xpocketmc\event\player\PlayerInteractEvent;
use xpocketmc\event\world\ChunkLoadEvent;
use xpocketmc\event\world\ChunkPopulateEvent;
use xpocketmc\event\world\ChunkUnloadEvent;
use xpocketmc\event\world\SpawnChangeEvent;
use xpocketmc\event\world\WorldDifficultyChangeEvent;
use xpocketmc\event\world\WorldDisplayNameChangeEvent;
use xpocketmc\event\world\WorldParticleEvent;
use xpocketmc\event\world\WorldSaveEvent;
use xpocketmc\event\world\WorldSoundEvent;
...
use xpocketmc\ServerConfigGroup;
use xpocketmc\utils\AssumptionFailedError;
use xpocketmc\utils\Limits;
use xpocketmc\utils\ReversePriorityQueue;
use xpocketmc\world\biome\Biome;
...
use xpocketmc\world\generator\GeneratorRegisterTask;
use xpocketmc\world\generator\GeneratorUnregisterTask;
use xpocketmc\world\generator\PopulationTask;
use xpocketmc\world\light\BlockLightUpdate;
use xpocketmc\world\light\LightPopulationTask;
use xpocketmc\world\light\SkyLightUpdate;
use xpocketmc\world\particle\BlockBreakParticle;
use xpocketmc\world\particle\Particle;
use xpocketmc\world\sound\BlockPlaceSound;
...
class World implements ChunkManager{
	...
	/** @phpstan-var class-string<\xpocketmc\world\generator\Generator> */
	...
	public function __construct(
		...
		$this->server->getLogger()->info($this->server->getLanguage()->translate(KnownTranslationFactory::xpocketmc_level_preparing($this->displayName)));
		...
		if($cfg->getPropertyInt("chunk-ticking.per-tick", 40) <= 0){
			...
			$this->logger->warning("\"chunk-ticking.per-tick\" setting is deprecated, but you've used it to disable chunk ticking. Set \"chunk-ticking.tick-radius\" to 0 in \"xpocketmc.yml\" instead.");
