...
use xpocketmc\block\UnknownBlock;
use xpocketmc\block\VanillaBlocks;
use xpocketmc\command\CommandSender;
use xpocketmc\crafting\CraftingGrid;
use xpocketmc\data\java\GameModeIdMap;
use xpocketmc\entity\animation\Animation;
use xpocketmc\entity\animation\ArmSwingAnimation;
use xpocketmc\entity\animation\CriticalHitAnimation;
...
use xpocketmc\event\inventory\InventoryCloseEvent;
use xpocketmc\event\inventory\InventoryOpenEvent;
use xpocketmc\event\player\PlayerBedEnterEvent;
use xpocketmc\event\player\PlayerBedLeaveEvent;
use xpocketmc\event\player\PlayerBlockPickEvent;
use xpocketmc\event\player\PlayerChangeSkinEvent;
use xpocketmc\event\player\PlayerChatEvent;
use xpocketmc\event\player\PlayerDeathEvent;
use xpocketmc\event\player\PlayerDisplayNameChangeEvent;
use xpocketmc\event\player\PlayerDropItemEvent;
use xpocketmc\event\player\PlayerEmoteEvent;
use xpocketmc\event\player\PlayerEntityInteractEvent;
use xpocketmc\event\player\PlayerExhaustEvent;
use xpocketmc\event\player\PlayerGameModeChangeEvent;
use xpocketmc\event\player\PlayerInteractEvent;
use xpocketmc\event\player\PlayerItemConsumeEvent;
use xpocketmc\event\player\PlayerItemHeldEvent;
use xpocketmc\event\player\PlayerItemUseEvent;
use xpocketmc\event\player\PlayerJoinEvent;
use xpocketmc\event\player\PlayerJumpEvent;
use xpocketmc\event\player\PlayerKickEvent;
use xpocketmc\event\player\PlayerMissSwingEvent;
use xpocketmc\event\player\PlayerMoveEvent;
use xpocketmc\event\player\PlayerPostChunkSendEvent;
use xpocketmc\event\player\PlayerQuitEvent;
use xpocketmc\event\player\PlayerRespawnEvent;
use xpocketmc\event\player\PlayerToggleFlightEvent;
use xpocketmc\event\player\PlayerToggleGlideEvent;
use xpocketmc\event\player\PlayerToggleSneakEvent;
use xpocketmc\event\player\PlayerToggleSprintEvent;
use xpocketmc\event\player\PlayerToggleSwimEvent;
use xpocketmc\event\player\PlayerTransferEvent;
use xpocketmc\event\player\PlayerViewDistanceChangeEvent;
use xpocketmc\form\Form;
use xpocketmc\form\FormValidationException;
use xpocketmc\inventory\CallbackInventoryListener;
use xpocketmc\inventory\CreativeInventory;
use xpocketmc\inventory\Inventory;
use xpocketmc\inventory\PlayerCraftingInventory;
use xpocketmc\inventory\PlayerCursorInventory;
use xpocketmc\inventory\TemporaryInventory;
use xpocketmc\inventory\transaction\action\DropItemAction;
use xpocketmc\inventory\transaction\InventoryTransaction;
use xpocketmc\inventory\transaction\TransactionBuilder;
use xpocketmc\inventory\transaction\TransactionCancelledException;
use xpocketmc\inventory\transaction\TransactionValidationException;
use xpocketmc\item\ConsumableItem;
use xpocketmc\item\Durable;
use xpocketmc\item\enchantment\EnchantmentInstance;
use xpocketmc\item\enchantment\MeleeWeaponEnchantment;
use xpocketmc\item\Item;
use xpocketmc\item\ItemUseResult;
use xpocketmc\item\Releasable;
...
use xpocketmc\player\chat\StandardChatFormatter;
...
use xpocketmc\world\ChunkLoader;
use xpocketmc\world\ChunkTicker;
use xpocketmc\world\format\Chunk;
use xpocketmc\world\Position;
use xpocketmc\world\sound\EntityAttackNoDamageSound;
use xpocketmc\world\sound\EntityAttackSound;
use xpocketmc\world\sound\FireExtinguishSound;
use xpocketmc\world\sound\ItemBreakSound;
use xpocketmc\world\sound\Sound;
use xpocketmc\world\World;
use xpocketmc\YmlServerProperties;
...
class Player extends Human implements CommandSender, ChunkListener, IPlayer{
	...
	public function transfer(string $address, int $port = 19132, Translatable|string|null $message = null) : bool{
		$ev = new PlayerTransferEvent($this, $address, $port, $message ?? KnownTranslationFactory::xpocketmc_disconnect_transfer());
		$ev->call();
		if(!$ev->isCancelled()){
	...
	public function respawn() : void{
		if($this->server->isHardcore()){
			if($this->kick(KnownTranslationFactory::xpocketmc_disconnect_ban(KnownTranslationFactory::xpocketmc_disconnect_ban_hardcore()))){ //this allows plugins to prevent the ban by cancelling PlayerKickEvent
	...
	protected function actuallyRespawn() : void{
		...
		$spawn->getWorld()->requestSafeSpawn($spawn)->onCompletion(
			...
			function() : void{
				if($this->isConnected()){
					$this->getNetworkSession()->disconnectWithError(KnownTranslationFactory::xpocketmc_disconnect_error_respawn());
				}
			}
		);
	}
