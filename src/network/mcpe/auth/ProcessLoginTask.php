...
class ProcessLoginTask extends AsyncTask{
	...
	private function validateToken(string $jwt, ?string &$currentPublicKey, bool $first = false) : void{
		...
		if($currentPublicKey === null){
			if(!$first){
				throw new VerifyLoginException("Missing JWT public key", KnownTranslationFactory::xpocketmc_disconnect_invalidSession_missingKey());
			}
			...
			throw new VerifyLoginException("Invalid JWT signature", KnownTranslationFactory::xpocketmc_disconnect_invalidSession_badSignature());
		}
		...
		if(isset($claims->nbf) && $claims->nbf > $time + self::CLOCK_DRIFT_MAX){
			throw new VerifyLoginException("JWT not yet valid", KnownTranslationFactory::xpocketmc_disconnect_invalidSession_tooEarly());
		}

		if(isset($claims->exp) && $claims->exp < $time - self::CLOCK_DRIFT_MAX){
			throw new VerifyLoginException("JWT expired", KnownTranslationFactory::xpocketmc_disconnect_invalidSession_tooLate());
		}
