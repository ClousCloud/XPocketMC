...
use xpocketmc\event\player\PlayerResourcePackOfferEvent;
use xpocketmc\event\server\DataPacketDecodeEvent;
use xpocketmc\event\server\DataPacketReceiveEvent;
...
use xpocketmc\player\UsedChunkStatus;
...
class NetworkSession{
	...
	public function __construct(
		...
		$this->logger->info($this->server->getLanguage()->translate(KnownTranslationFactory::xpocketmc_network_session_open()));
	}
	...
	private function onSessionStartSuccess() : void{
		...
		$this->setHandler(new LoginPacketHandler(
			...
			function(PlayerInfo $info) : void{
				$this->info = $info;
				$this->logger->info($this->server->getLanguage()->translate(KnownTranslationFactory::xpocketmc_network_session_playerName(TextFormat::AQUA . $info->getUsername() . TextFormat::RESET)));
		...
		$this->server->createPlayer($this, $this->info, $this->authenticated, $this->cachedOfflinePlayerData)->onCompletion(
			...
			function() : void{
				...
				$this->disconnectWithError(
					...
					disconnectScreenMessage: KnownTranslationFactory::xpocketmc_disconnect_error_internal()
				);
			}
		);
	}
	...
	private function tryDisconnect(\Closure $func, Translatable|string $reason) : void{
		if($this->connected && !$this->disconnectGuard){
			...
			$this->logger->info($this->server->getLanguage()->translate(KnownTranslationFactory::xpocketmc_network_session_close($reason)));
		}
	}
	...
	public function disconnectWithError(Translatable|string $reason, Translatable|string|null $disconnectScreenMessage = null) : void{
		...
		$this->disconnect(
			reason: KnownTranslationFactory::xpocketmc_disconnect_error($reason, $errorId)->prefix(TextFormat::RED),
			disconnectScreenMessage: KnownTranslationFactory::xpocketmc_disconnect_error($disconnectScreenMessage ?? $reason, $errorId),
		);
	}
	...
	public function disconnectIncompatibleProtocol(int $protocolVersion) : void{
		$this->tryDisconnect(
			...
			KnownTranslationFactory::xpocketmc_disconnect_incompatibleProtocol((string) $protocolVersion)
		);
	}
	...
	public function transfer(string $ip, int $port, Translatable|string|null $reason = null) : void{
		$reason ??= KnownTranslationFactory::xpocketmc_disconnect_transfer();
	...
	private function setAuthenticationStatus(bool $authenticated, bool $authRequired, Translatable|string|null $error, ?string $clientPubKey) : void{
		...
		if($error !== null){
			$this->disconnectWithError(
				reason: KnownTranslationFactory::xpocketmc_disconnect_invalidSession($error),
				disconnectScreenMessage: KnownTranslationFactory::xpocketmc_disconnect_error_authentication()
			);
	...
	public function prepareClientTranslatableMessage(Translatable $message) : array{
		...
		return [$language->translateString($message->getText(), $parameters, "xpocketmc."), $parameters];
	}
	...
	public function tick() : void{
		...
		if($this->info === null){
			if(time() >= $this->connectTime + 10){
				$this->disconnectWithError(KnownTranslationFactory::xpocketmc_disconnect_error_loginTimeout());
			}
