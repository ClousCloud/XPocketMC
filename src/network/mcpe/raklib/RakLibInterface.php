	...
	public function onClientDisconnect(int $sessionId, int $reason) : void{
		if(isset($this->sessions[$sessionId])){
			...
			$session->onClientDisconnect(match($reason){
				DisconnectReason::CLIENT_DISCONNECT => KnownTranslationFactory::xpocketmc_disconnect_clientDisconnect(),
				DisconnectReason::PEER_TIMEOUT => KnownTranslationFactory::xpocketmc_disconnect_error_timeout(),
				DisconnectReason::CLIENT_RECONNECT => KnownTranslationFactory::xpocketmc_disconnect_clientReconnect(),
		...
		if(isset($this->sessions[$sessionId])){
			...
			try{
				...
				$session->disconnectWithError(
					...
					disconnectScreenMessage: KnownTranslationFactory::xpocketmc_disconnect_error_badPacket()
				);
