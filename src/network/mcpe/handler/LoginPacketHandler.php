...
class LoginPacketHandler extends PacketHandler{
	...
	public function handleLogin(LoginPacket $packet) : bool{
		...
		if(!$this->server->isWhitelisted($playerInfo->getUsername())){
			$ev->setKickFlag(PlayerPreLoginEvent::KICK_FLAG_SERVER_WHITELISTED, KnownTranslationFactory::xpocketmc_disconnect_whitelisted());
		}

		$banMessage = null;
		if(($banEntry = $this->server->getNameBans()->getEntry($playerInfo->getUsername())) !== null){
			$banReason = $banEntry->getReason();
			$banMessage = $banReason === "" ? KnownTranslationFactory::xpocketmc_disconnect_ban_noReason() : KnownTranslationFactory::xpocketmc_disconnect_ban($banReason);
		}elseif(($banEntry = $this->server->getIPBans()->getEntry($this->session->getIp())) !== null){
			$banReason = $banEntry->getReason();
			$banMessage = KnownTranslationFactory::xpocketmc_disconnect_ban($banReason !== "" ? $banReason : KnownTranslationFactory::xpocketmc_disconnect_ban_ip());
		}
		if($banMessage !== null){
