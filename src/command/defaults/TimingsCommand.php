...
use xpocketmc\scheduler\BulkCurlTask;
use xpocketmc\scheduler\BulkCurlTaskOperation;
use xpocketmc\timings\TimingsHandler;
use xpocketmc\utils\InternetException;
use xpocketmc\utils\InternetRequestResult;
...
class TimingsCommand extends VanillaCommand{
	...
	public function __construct(){
		parent::__construct(
			"timings",
			KnownTranslationFactory::xpocketmc_command_timings_description(),
			KnownTranslationFactory::xpocketmc_command_timings_usage()
		);
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		if($mode === "on"){
			if(TimingsHandler::isEnabled()){
				$sender->sendMessage(KnownTranslationFactory::xpocketmc_command_timings_alreadyEnabled());
				return true;
			}
			TimingsHandler::setEnabled();
			Command::broadcastCommandMessage($sender, KnownTranslationFactory::xpocketmc_command_timings_enable());

			return true;
		}elseif($mode === "off"){
			TimingsHandler::setEnabled(false);
			Command::broadcastCommandMessage($sender, KnownTranslationFactory::xpocketmc_command_timings_disable());
			return true;
		}

		if(!TimingsHandler::isEnabled()){
			$sender->sendMessage(KnownTranslationFactory::xpocketmc_command_timings_timingsDisabled());
		...
		if($mode === "reset"){
			...
			Command::broadcastCommandMessage($sender, KnownTranslationFactory::xpocketmc_command_timings_reset());
			...
			if($paste){
				fseek($fileTimings, 0);
				$data = [
					"browser" => $agent = $sender->getServer()->getName() . " " . $sender->getServer()->getxpocketmcVersion(),
				...
				$sender->getServer()->getAsyncPool()->submitTask(new BulkCurlTask(
					...
					function(array $results) use ($sender, $host) : void{
						...
						if(is_array($response) && isset($response["id"])){
							Command::broadcastCommandMessage($sender, KnownTranslationFactory::xpocketmc_command_timings_timingsRead(
							...
							Command::broadcastCommandMessage($sender, KnownTranslationFactory::xpocketmc_command_timings_pasteError());
						}
					}
				));
			}else{
				fclose($fileTimings);
				Command::broadcastCommandMessage($sender, KnownTranslationFactory::xpocketmc_command_timings_timingsWrite($timings));
			}
		}else{
			throw new InvalidCommandSyntaxException();
		}
