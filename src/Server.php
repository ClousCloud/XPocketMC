...
use xpocketmc\command\SimpleCommandMap;
use xpocketmc\console\ConsoleCommandSender;
use xpocketmc\console\ConsoleReaderChildProcessDaemon;
use xpocketmc\crafting\CraftingManager;
use xpocketmc\crafting\CraftingManagerFromDataHelper;
use xpocketmc\crash\CrashDump;
use xpocketmc\crash\CrashDumpRenderer;
use xpocketmc\entity\EntityDataHelper;
use xpocketmc\entity\Location;
use xpocketmc\event\HandlerListManager;
use xpocketmc\event\player\PlayerCreationEvent;
use xpocketmc\event\player\PlayerDataSaveEvent;
use xpocketmc\event\player\PlayerLoginEvent;
use xpocketmc\event\server\CommandEvent;
use xpocketmc\event\server\QueryRegenerateEvent;
...
use xpocketmc\player\OfflinePlayer;
use xpocketmc\player\Player;
use xpocketmc\player\PlayerDataLoadException;
use xpocketmc\player\PlayerDataProvider;
use xpocketmc\player\PlayerDataSaveException;
use xpocketmc\player\PlayerInfo;
use xpocketmc\plugin\PharPluginLoader;
use xpocketmc\plugin\PluginEnableOrder;
use xpocketmc\plugin\PluginGraylist;
use xpocketmc\plugin\PluginManager;
use xpocketmc\plugin\PluginOwned;
use xpocketmc\plugin\ScriptPluginLoader;
...
use xpocketmc\updater\UpdateChecker;
use xpocketmc\utils\AssumptionFailedError;
use xpocketmc\utils\BroadcastLoggerForwarder;
use xpocketmc\utils\Config;
use xpocketmc\utils\Filesystem;
use xpocketmc\utils\Internet;
use xpocketmc\utils\MainLogger;
use xpocketmc\utils\NotCloneable;
use xpocketmc\utils\NotSerializable;
use xpocketmc\utils\Process;
use xpocketmc\utils\SignalHandler;
use xpocketmc\utils\Terminal;
use xpocketmc\utils\TextFormat;
use xpocketmc\utils\Utils;
use xpocketmc\world\format\io\WorldProviderManager;
use xpocketmc\world\format\io\WritableWorldProviderManagerEntry;
use xpocketmc\world\generator\Generator;
use xpocketmc\world\generator\GeneratorManager;
use xpocketmc\world\generator\InvalidGeneratorOptionsException;
use xpocketmc\world\Position;
use xpocketmc\world\World;
use xpocketmc\world\WorldCreationOptions;
use xpocketmc\world\WorldManager;
use xpocketmc\YmlServerProperties as Yml;
...
class Server{
	...
	public const BROADCAST_CHANNEL_ADMINISTRATIVE = "xpocketmc.broadcast.admin";
	public const BROADCAST_CHANNEL_USERS = "xpocketmc.broadcast.user";
	...
	public function getxpocketmcVersion() : string{
	...
	public function getFilePath() : string{
		return \xpocketmc\PATH;
	}

	public function getResourcePath() : string{
		return \xpocketmc\RESOURCE_PATH;
	}
	...
	public function getOfflinePlayerData(string $name) : ?CompoundTag{
		return Timings::$syncPlayerDataLoad->time(function() use ($name) : ?CompoundTag{
			try{
				...
				$this->logger->error($this->language->translate(KnownTranslationFactory::xpocketmc_data_playerCorrupted($name)));
				return null;
			}
		});
	}
		...
		if(!$ev->isCancelled()){
			Timings::$syncPlayerDataSave->time(function() use ($name, $ev) : void{
				try{
					...
					$this->logger->critical($this->language->translate(KnownTranslationFactory::xpocketmc_data_saveError($name, $e->getMessage())));
		...
		if($playerPos === null){ //new player or no valid position due to world not being loaded
			$world->requestSafeSpawn()->onCompletion(
				...
				function() use ($playerPromiseResolver, $session) : void{
					if($session->isConnected()){
						$session->disconnectWithError(KnownTranslationFactory::xpocketmc_disconnect_error_respawn());
					}
					$playerPromiseResolver->reject();
				}
			);
	...
	public function __construct(
		...
		try{
			...
			$xpocketmcYmlPath = Path::join($this->dataPath, "xpocketmc.yml");
			if(!file_exists($xpocketmcYmlPath)){
				$content = Filesystem::fileGetContents(Path::join(\xpocketmc\RESOURCE_PATH, "xpocketmc.yml"));
				if(VersionInfo::IS_DEVELOPMENT_BUILD){
					$content = str_replace("preferred-channel: stable", "preferred-channel: beta", $content);
				}
				@file_put_contents($xpocketmcYmlPath, $content);
			}

			$this->configGroup = new ServerConfigGroup(
				new Config($xpocketmcYmlPath, Config::YAML, []),
			...
			if(VersionInfo::IS_DEVELOPMENT_BUILD){
				if(!$this->configGroup->getPropertyBool(Yml::SETTINGS_ENABLE_DEV_BUILDS, false)){
					$this->logger->emergency($this->language->translate(KnownTranslationFactory::xpocketmc_server_devBuild_error1(VersionInfo::NAME)));
					$this->logger->emergency($this->language->translate(KnownTranslationFactory::xpocketmc_server_devBuild_error2()));
					$this->logger->emergency($this->language->translate(KnownTranslationFactory::xpocketmc_server_devBuild_error3()));
					$this->logger->emergency($this->language->translate(KnownTranslationFactory::xpocketmc_server_devBuild_error4(Yml::SETTINGS_ENABLE_DEV_BUILDS)));
					$this->logger->emergency($this->language->translate(KnownTranslationFactory::xpocketmc_server_devBuild_error5("https://github.com/pmmp/xpocketmc-MP/releases")));
				...
				$this->logger->warning($this->language->translate(KnownTranslationFactory::xpocketmc_server_devBuild_warning1(VersionInfo::NAME)));
				$this->logger->warning($this->language->translate(KnownTranslationFactory::xpocketmc_server_devBuild_warning2()));
				$this->logger->warning($this->language->translate(KnownTranslationFactory::xpocketmc_server_devBuild_warning3()));
				$this->logger->warning(str_repeat("-", 40));
			}

			$this->memoryManager = new MemoryManager($this);

			$this->logger->info($this->language->translate(KnownTranslationFactory::xpocketmc_server_start(TextFormat::AQUA . $this->getVersion() . TextFormat::RESET)));
			...
			if($this->onlineMode){
				$this->logger->info($this->language->translate(KnownTranslationFactory::xpocketmc_server_auth_enabled()));
			}else{
				$this->logger->warning($this->language->translate(KnownTranslationFactory::xpocketmc_server_auth_disabled()));
				$this->logger->warning($this->language->translate(KnownTranslationFactory::xpocketmc_server_authWarning()));
				$this->logger->warning($this->language->translate(KnownTranslationFactory::xpocketmc_server_authProperty_disabled()));
			}
			...
			@cli_set_process_title($this->getName() . " " . $this->getxpocketmcVersion());
			...
			$this->logger->info($this->language->translate(KnownTranslationFactory::xpocketmc_server_info(
				$this->getName(),
				(VersionInfo::IS_DEVELOPMENT_BUILD ? TextFormat::YELLOW : "") . $this->getxpocketmcVersion() . TextFormat::RESET
			)));
			$this->logger->info($this->language->translate(KnownTranslationFactory::xpocketmc_server_license($this->getName())));
			...
			$this->craftingManager = CraftingManagerFromDataHelper::make(Path::join(\xpocketmc\BEDROCK_DATA_PATH, "recipes"));
			...
			if(!file_exists($graylistFile)){
				copy(Path::join(\xpocketmc\RESOURCE_PATH, 'plugin_list.yml'), $graylistFile);
			}
			try{
			...
			if(
				...
				$this->logger->warning($this->language->translate(KnownTranslationFactory::xpocketmc_level_badDefaultFormat($formatName)));
			}
			...
			if($loadErrorCount > 0){
				$this->logger->emergency($this->language->translate(KnownTranslationFactory::xpocketmc_plugin_someLoadErrors()));
				$this->forceShutdownExit();
				return;
			}
			if(!$this->enablePlugins(PluginEnableOrder::STARTUP)){
				$this->logger->emergency($this->language->translate(KnownTranslationFactory::xpocketmc_plugin_someEnableErrors()));
				$this->forceShutdownExit();
				return;
			}
			...
			$this->logger->info($this->language->translate(KnownTranslationFactory::xpocketmc_server_defaultGameMode($this->getGamemode()->getTranslatableName())));
			$this->logger->info($this->language->translate(KnownTranslationFactory::xpocketmc_server_donate(TextFormat::AQUA . "https://patreon.com/xpocketmcmp" . TextFormat::RESET)));
			$this->logger->info($this->language->translate(KnownTranslationFactory::xpocketmc_server_startFinished(strval(round(microtime(true) - $this->startTime, 3)))));
	...
	private function startupPrepareWorlds() : bool{
		$getGenerator = function(string $generatorName, string $generatorOptions, string $worldName) : ?string{
			...
			if($generatorEntry === null){
				$this->logger->error($this->language->translate(KnownTranslationFactory::xpocketmc_level_generationError(
					$worldName,
					KnownTranslationFactory::xpocketmc_level_unknownGenerator($generatorName)
				)));
				return null;
			}
			try{
				...
				$this->logger->error($this->language->translate(KnownTranslationFactory::xpocketmc_level_generationError(
					$worldName,
					KnownTranslationFactory::xpocketmc_level_invalidGeneratorOptions($generatorOptions, $generatorName, $e->getMessage())
				)));
				return null;
			}
		...
		if($this->worldManager->getDefaultWorld() === null){
			...
			if(!$this->worldManager->loadWorld($default, true)){
				if($this->worldManager->isWorldGenerated($default)){
					$this->logger->emergency($this->language->translate(KnownTranslationFactory::xpocketmc_level_defaultError()));
	...
	private function startupPrepareConnectableNetworkInterfaces(
		...
		try{
			...
			$this->logger->emergency($this->language->translate(KnownTranslationFactory::xpocketmc_server_networkStartFailed(
				$ip,
				(string) $port,
				$e->getMessage()
			)));
		...
		if($rakLibRegistered){
			$this->logger->info($this->language->translate(KnownTranslationFactory::xpocketmc_server_networkStart($prettyIp, (string) $port)));
		}
		if($useQuery){
			if(!$rakLibRegistered){
			...
			$this->logger->info($this->language->translate(KnownTranslationFactory::xpocketmc_server_query_running($prettyIp, (string) $port)));
		}
		return true;
	}
	...
	public function forceShutdown() : void{
		...
		if($this->isRunning){
			$this->logger->emergency($this->language->translate(KnownTranslationFactory::xpocketmc_server_forcingShutdown()));
		}
		try{
			if(!$this->isRunning()){
	...
	public function crashDump() : void{
		...
		try{
			$this->logger->emergency($this->language->translate(KnownTranslationFactory::xpocketmc_crash_create()));
			$dump = new CrashDump($this, $this->pluginManager ?? null);

			$crashDumpPath = $this->writeCrashDumpFile($dump);

			$this->logger->emergency($this->language->translate(KnownTranslationFactory::xpocketmc_crash_submit($crashDumpPath)));
			...
			if($this->configGroup->getPropertyBool(Yml::AUTO_REPORT_ENABLED, true)){
				...
				if($report){
					...
					$reply = Internet::postURL($url, [
						"report" => "yes",
						"name" => $this->getName() . " " . $this->getxpocketmcVersion(),
						"email" => "crash@xpocketmc.net",
					...
					if($reply !== null && is_object($data = json_decode($reply->getBody()))){
						if(isset($data->crashId) && is_int($data->crashId) && isset($data->crashUrl) && is_string($data->crashUrl)){
							...
							$this->logger->emergency($this->language->translate(KnownTranslationFactory::xpocketmc_crash_archive($reportUrl, (string) $reportId)));
			...
			$this->logger->logException($e);
			try{
				$this->logger->critical($this->language->translate(KnownTranslationFactory::xpocketmc_crash_error($e->getMessage())));
			}catch(\Throwable $e){}
		}
	...
	public function addOnlinePlayer(Player $player) : bool{
		...
		$this->logger->info($this->language->translate(KnownTranslationFactory::xpocketmc_player_logIn(
	...
	private function tick() : void{
		...
		if(($this->tickCounter % self::TICKS_PER_TPS_OVERLOAD_WARNING) === 0 && $this->getTicksPerSecondAverage() < self::TPS_OVERLOAD_WARNING_THRESHOLD){
			$this->logger->warning($this->language->translate(KnownTranslationFactory::xpocketmc_server_tickOverload()));
		}
