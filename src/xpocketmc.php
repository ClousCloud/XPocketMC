...
namespace xpocketmc {
	...
	use xpocketmc\utils\ServerKiller;
	use xpocketmc\utils\Terminal;
	use xpocketmc\utils\Timezone;
	use xpocketmc\utils\Utils;
	use xpocketmc\wizard\SetupWizard;
	...
	function check_platform_dependencies(){
		...
		$extensions = [
			"chunkutils2" => "xpocketmc ChunkUtils v2",
			"curl" => "cURL",
			"crypto" => "php-crypto",
			"ctype" => "ctype",
			"date" => "Date",
		...
		if(extension_loaded("xpocketmc")){
			$messages[] = "The native xpocketmc extension is no longer supported.";
		}
...
JIT_WARNING
	...
	function server(){
		...
		$composerGitHash = InstalledVersions::getReference('xpocketmc/xpocketmc-mp');
